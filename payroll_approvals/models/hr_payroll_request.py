...
class HrPayslip(models.Model):
    ...
    approver_ids = fields.One2many('payroll.approval.approver', 'task_id', string="Approvers",
    ...
    def action_approve(self, approver=None):
        ...
        activity_type = self.env.ref('payroll_approvals.mail_activity_data_approval_payroll')
    ...
    def _update_next_approvers(self, new_status, approver, only_next_approver, cancel_activities=False):
        approvers_updated = self.env['payroll.approval.approver']
    ...
    def _get_user_approval_activities(self, user):
        return self.env['mail.activity'].search([
            ('res_model', '=', 'hr.payslip'),
            ('res_id', 'in', self.ids),
            ('activity_type_id', '=', self.env.ref('payroll_approvals.mail_activity_data_approval_payroll').id),
            ('user_id', '=', user.id)
        ])
...
class PayrollApprovalApprover(models.Model):
    _name ='payroll.approval.approver'
    ...
    task_id = fields.Many2one('hr.payslip', string="Task", ondelete='cascade', check_company=True)
            ...
            if approver.status == 'pending':
                ...
                if not existing:
                    approver.task_id.activity_schedule(
                        'payroll_approvals.mail_activity_data_approval_payroll',
                        user_id=approver.user_id.id
                    )
