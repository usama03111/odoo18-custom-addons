...
class Meeting(models.Model):
    ...
    def _compute_dates(self):
        ...
        for meeting in self:
            ...
            elif meeting.start and meeting.stop:
                meeting.duration = self._get_duration(meeting.start, meeting.stop)
            elif meeting.start and not meeting.stop:
...
class ScrumMeeting(models.Model):
    ...
    def validate(self, meeting):
        ...
        vals_line['date'] = meeting.start_date or meeting.start
        vals_line['meeting_id'] = meeting.id
        vals_line['unit_amount'] = meeting.allday and 8 or meeting.duration
        vals_line['account_id'] = project.account_id.id
        # vals_line['account_id'] = project.analytic_account_id.id
...
class ProjectScrumProductBacklog(models.Model):
    ...
    @api.depends('tasks_id',
                 'tasks_id.effective_hours', 'tasks_id.allocated_hours')
    def _compute_hours(self):
        for backlog in self:
            ...
            for task in backlog.tasks_id:
                ...
                tot += task.allocated_hours
                prog += task.allocated_hours * task.progress / 100.0
                progre = prog * 100
            if tot > 0:
                progress = (progre / tot * 100)
                # progress = round(prog / tot * 100)
...
class MailAlias(models.Model):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get('alias_name'):
                alias_name = vals['alias_name'].strip().lower().replace(' ', '_')
                existing = self.search([('alias_name', '=', alias_name)], limit=1)
                if existing:
                    raise ValidationError(_("Alias name %s already exists") % alias_name)
            ...
            if parent_model_name:
                ...
                vals['alias_parent_model_id'] = parent_model.id
