...
class AIConfig(models.Model):
    ...
    hide_prompt = fields.Boolean('Hide options', compute='_compute_hide_prompt')

    @api.depends('operation_key', 'ai_model_id.key')
    def _compute_hide_prompt(self):
        for rec in self:
            if rec.operation_key == 'chat_completions' and rec.ai_model_id.key not in USE_COMPLETIONS:
                rec.hide_prompt = False
            else:
                rec.hide_prompt = True
    ...
    def execute_openai(self, prompt, **kwargs):
        ...
        if self.operation_key == 'chat_completions':
            if self.hide_prompt:
                message_ids = self.get_messages(kwargs.get('conversation'))
                if not message_ids:
    ...
    def get_messages(self, conversation, ttype='text'):
        ...
        domain.extend(self.get_messages_extra_domain(conversation, ttype))
        return Message.search(domain, limit=self.message_number)

    def get_messages_extra_domain(self, conversation, ttype='text'):
        self.ensure_one()
        return []
    ...
    def get_url(self) -> str:
        ...
        out = ''
        if self.provider == 'openai':
            operation_key = str(self.operation_key)
            if operation_key == 'completions' and self.ai_model_id.key not in USE_COMPLETIONS:
                operation_key = 'chat_completions'
            out = f'{self.endpoint.strip("/")}/{operation_key.replace("_", "/")}'
        return out
    ...
    def get_body(self, data_to_process, **kwargs) -> dict:
        ...
        if self.provider == 'openai':
            ...
            if self.operation_key == 'completions':
                ...
                data_to_process2 = []
                if self.command:
                    data_to_process2 = [{'role': 'system', 'content': self.get_command(**kwargs)}]
                data_to_process2.extend(data_to_process)
                out.update({
                    'presence_penalty': self.presence_penalty,
                    'frequency_penalty': self.frequency_penalty,
                    'messages': data_to_process2,
                })
            elif self.operation_key == 'audio_transcriptions':
    ...
    def can_edit_request_text(self):
        ...
        out = True
        if self.provider == 'openai':
            out = self.operation_key not in ['audio_transcriptions']
        return out
