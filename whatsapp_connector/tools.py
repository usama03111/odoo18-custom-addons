...
def phone_format(number, country_id=None, formatted=False, raise_error=True):
    ...
    try:
        number = number.lstrip(' +')
        if number.startswith('52'):  # mexico patch
            is_mexico = True
            if not number.startswith('521'):
                raise UserError(str(number) + _(' Invalid number.'))
            number = number.replace('521', '52', 1)
    ...
    if is_mexico:  # mexico patch
        format_number = format_number.replace('52', '52 1', 1)
...
def image2jpg(env, content):
    ...
    try:
        ...
        ret = image_process(encoded_image, size=size, quality=80, output_format='JPEG')
    except Exception:
        ret = False
